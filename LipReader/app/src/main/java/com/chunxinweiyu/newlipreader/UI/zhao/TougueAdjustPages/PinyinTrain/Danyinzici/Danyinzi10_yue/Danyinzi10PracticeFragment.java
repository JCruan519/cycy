...
public class Danyinzi10PracticeFragment extends Fragment {
    //示范视频标签
    private String               demoVideoName         = "yue";
    //上传视频标签
    private String               uploadVideoName       = "danyinzi10";
    //弹窗对象
    private CongratulationDialog mCongratulationDialog;
    //当前模块对应的视频播放控制器
    private VideoPlayer          danyinzi10VideoPlayer   = new VideoPlayer();
    //当前模块对应的视频录制控制器
    private VideoRecorder        danyinzi10VideoRecorder = new VideoRecorder();
    //最后显示在页面上的分数
    private double               finalScore;
    //分数字符串
    private TextView             scoreString;

    public Danyinzi10PracticeFragment() {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        return inflater.inflate(R.layout.fragment_danyinzi10_practice, container, false);
    }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        //设置这个模块返回的视频尾部标签
        danyinzi10VideoRecorder.label = uploadVideoName;
        //相机预览界面设置
        FrameLayout camPreview = view.findViewById(R.id.camera_preview);
        danyinzi10VideoRecorder.prepareCameraAndStartPreview(getContext(), camPreview);
        //进度条读条&录像倒计时
        ProgressBar progressBar;
        progressBar = view.findViewById(R.id.progress_bar);
        danyinzi10VideoRecorder.setCountDown(progressBar);
        //分数显示绑定
    ...
    public void onDestroy() {
        super.onDestroy();
        danyinzi10VideoRecorder.releaseCameraAndStopPreview();
    }
    ...
    private void setReplayVideoBtn(final View replayVideoBtn){
        replayVideoBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                danyinzi10VideoPlayer.restartVideo(demoVideoName);
            }
        });
    ...
    private void setReplayRecordBtn(final View replayRecordBtn){
        replayRecordBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                danyinzi10VideoPlayer.stratRecordedPreview();
            }
        });
    ...
    private void setStartPracticeBtn(final View startPracticeBtn){
        startPracticeBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                danyinzi10VideoRecorder.startMediaCorder(startPracticeBtn);
                danyinzi10VideoPlayer.recordDone = true;
                waitStatusUpdate();
    ...
    private void setShengmu1VideoPlayer(SurfaceView videoPlaysv){
        danyinzi10VideoPlayer.slowModePlayer(videoPlaysv, getActivity());
        danyinzi10VideoPlayer.setSurfaceHolder(videoPlaysv.getHolder(), demoVideoName);
    }
    ...
    private void flashScore(){
        new Thread(new Runnable() {
            ...
            public void run() {
                while (true){
                    double score = danyinzi10VideoRecorder.mNetworkFunctions.score;
                    if (score != 0 && danyinzi10VideoRecorder.mNetworkFunctions.uploadFlag){
                        finalScore = score;
                        danyinzi10VideoRecorder.mNetworkFunctions.score = 0;
