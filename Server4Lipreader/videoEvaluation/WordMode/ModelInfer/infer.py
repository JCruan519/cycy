...
import videoEvaluation.WordMode.ModelInfer.opts as opts
from videoEvaluation.WordMode.ModelInfer.opts import parser
...
def main():
    ...
    if args.resume:
        ...
        if os.path.isfile(args.resume):
            ...
            # checkpoint['state_dict'].pop('module.new_fc.weight')
            # checkpoint['state_dict'].pop('module.new_fc.bias')
    ...
    if args.mode != 'test':
        lip_dict_train, video_list_train = opts.file_deal(opts.TRAIN_DATA, opts.TRAIN_LABEL)
        lip_dict_val, video_list_val = opts.file_deal(opts.VAL_DATA, opts.VAL_LABEL)
        # train_num = int(len(video_list) * 0.95)
        train_loader = torch.utils.data.DataLoader(
            TSNDataSet(opts.TRAIN_DATA, args.mode, num_segments=args.num_segments,
                       img_size=args.img_size,
                       lip_dict=lip_dict_train,
        ...
        val_loader = torch.utils.data.DataLoader(
            TSNDataSet(opts.VAL_DATA, args.mode, num_segments=args.num_segments,
                       img_size=args.img_size,
                       lip_dict=lip_dict_val,
        ...
        lip_dict, video_list = opts.file_deal(opts.TEST_DATA)
    ...
    if args.mode == 'test':
        if args.sub == 'sub':
            ...
            inferencefusion(test_loader, model)
        return
...
def inference(test_loader, model):
    ...
    words_list = [num2word(x) for x in final_list]  # 得到文字标签
    import pandas as pd
    result = pd.DataFrame()
...
def inferencefusion(test_loader, model):
    torch.set_grad_enabled(False)
    # switch to evaluate mode
    model.eval()
    flag = False
