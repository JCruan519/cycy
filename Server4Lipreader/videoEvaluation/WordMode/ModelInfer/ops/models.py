...
class TSN(nn.Module):
    ...
    def _prepare_base_model(self, base_model):
        ...
        if 'resnet' in base_model:
            ...
            # self.input_size = 136
            self.input_mean = [0.550212, 0.366118, 0.318551]
            self.input_std = [0.144855, 0.124059, 0.225]
            ...
            from videoEvaluation.WordMode.ModelInfer.archs.mobilenet_v2 import mobilenet_v2, InvertedResidual
            ...
            from videoEvaluation.WordMode.ModelInfer.archs.bn_inception import bninception
    ...
    def get_augmentation(self, flip=True):
        if self.modality == 'RGB':
            if flip:
                trans_compose = [GroupColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.1),
                                                       GroupRandomRotation(degrees=5),
                                                       GroupRandomAffine(translate=(0.05, 0.1)),
                                                       #GroupScale(int(self.scale_size)),
                                                       GroupRandomHorizontalFlip(is_flow=False)]

                return RandomApply(trans_compose, p=0.3)
            else:
                print('#' * 20, 'NO FLIP!!!')
...
if __name__ == '__main__':
    ...
    x = torch.tensor(torch.randn(1, 72, 136, 136))
    y = net(x)
    g = make_dot(y)
    g.view(filename=None, directory=None)
