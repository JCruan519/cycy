 ...
 */
public class Duoyinzici1PracticeFragment extends Fragment {
    //示范视频标签
    private String               demoVideoName = "exp1";
    //上传视频标签
    private String               uploadVideoName = "tongguo";
    //弹窗对象
    private CongratulationDialog mCongratulationDialog;
    //当前模块对应的视频播放控制器
    private VideoPlayer          duoyizici1VideoPlayer   = new VideoPlayer();
    //当前模块对应的视频录制控制器
    private VideoRecorder        duoyizici1VideoRecorder = new VideoRecorder();
    //最后显示在页面上的分数
    private double               finalScore;
    //分数字符串
    private TextView             scoreString;

    public Duoyinzici1PracticeFragment() {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        return inflater.inflate(R.layout.fragment_duoyinzici1_practice, container, false);
    }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        setDuoyizici1VideoPlayer(videoPlaysv);
        //设置这个模块返回的视频尾部标签
        duoyizici1VideoRecorder.label = uploadVideoName;
        //相机预览界面设置
        FrameLayout camPreview = view.findViewById(R.id.camera_preview);
        duoyizici1VideoRecorder.prepareCameraAndStartPreview(getContext(), camPreview);
        //进度条读条&录像倒计时
        ProgressBar progressBar;
        progressBar = view.findViewById(R.id.progress_bar);
        duoyizici1VideoRecorder.setCountDown(progressBar);
        //分数显示绑定
    ...
    public void onDestroy() {
        super.onDestroy();
        duoyizici1VideoRecorder.releaseCameraAndStopPreview();
    }
    ...
    private void setReplayVideoBtn(final View replayVideoBtn){
        replayVideoBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                duoyizici1VideoPlayer.restartVideo(demoVideoName);
            }
        });
    ...
    private void setReplayRecordBtn(final View replayRecordBtn){
        replayRecordBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                duoyizici1VideoPlayer.stratRecordedPreview();
            }
        });
    ...
    private void setStartPracticeBtn(final View startPracticeBtn){
        startPracticeBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                duoyizici1VideoRecorder.startMediaCorder(startPracticeBtn);
                duoyizici1VideoPlayer.recordDone = true;
                waitStatusUpdate();
    ...
    private void setDuoyizici1VideoPlayer(SurfaceView videoPlaysv){
        duoyizici1VideoPlayer.slowModePlayer(videoPlaysv, getActivity());
        duoyizici1VideoPlayer.setSurfaceHolder(videoPlaysv.getHolder(), demoVideoName);
    }
    ...
    private void flashScore(){
        new Thread(new Runnable() {
            ...
            public void run() {
                while (true){
                    double score = duoyizici1VideoRecorder.mNetworkFunctions.score;
                    if (score != 0 && duoyizici1VideoRecorder.mNetworkFunctions.uploadFlag){
                        finalScore = score;
                        duoyizici1VideoRecorder.mNetworkFunctions.score = 0;
