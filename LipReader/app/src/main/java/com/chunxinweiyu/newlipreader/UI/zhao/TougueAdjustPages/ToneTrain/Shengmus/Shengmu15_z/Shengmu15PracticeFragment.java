package com.chunxinweiyu.newlipreader.UI.zhao.TougueAdjustPages.ToneTrain.Shengmus.Shengmu15_z;
...
public class Shengmu15PracticeFragment extends Fragment {
    //示范视频标签
    private String               demoVideoName         = "z";
    //上传视频标签
    private String               uploadVideoName       = "shengmu_15";
    //弹窗对象
    private CongratulationDialog mCongratulationDialog;
    //当前模块对应的视频播放控制器
    private VideoPlayer          shengmu15VideoPlayer   = new VideoPlayer();
    //当前模块对应的视频录制控制器
    private VideoRecorder        shengmu15VideoRecorder = new VideoRecorder();
    //最后显示在页面上的分数
    private double               finalScore;
    //分数字符串
    private TextView             scoreString;

    public Shengmu15PracticeFragment() {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        return inflater.inflate(R.layout.fragment_shengmu15_practice, container, false);
    }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        //设置这个模块返回的视频尾部标签
        shengmu15VideoRecorder.label = uploadVideoName;
        //相机预览界面设置
        FrameLayout camPreview = view.findViewById(R.id.camera_preview);
        shengmu15VideoRecorder.prepareCameraAndStartPreview(getContext(), camPreview);
        //进度条读条&录像倒计时
        ProgressBar progressBar;
        progressBar = view.findViewById(R.id.progress_bar);
        shengmu15VideoRecorder.setCountDown(progressBar);
        //分数显示绑定
    ...
    public void onDestroy() {
        super.onDestroy();
        shengmu15VideoRecorder.releaseCameraAndStopPreview();
    }
    ...
    private void setReplayVideoBtn(final View replayVideoBtn){
        replayVideoBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                shengmu15VideoPlayer.restartVideo(demoVideoName);
            }
        });
    ...
    private void setReplayRecordBtn(final View replayRecordBtn){
        replayRecordBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                shengmu15VideoPlayer.stratRecordedPreview();
            }
        });
    ...
    private void setStartPracticeBtn(final View startPracticeBtn){
        startPracticeBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                shengmu15VideoRecorder.startMediaCorder(startPracticeBtn);
                shengmu15VideoPlayer.recordDone = true;
                waitStatusUpdate();
    ...
    private void setShengmu1VideoPlayer(SurfaceView videoPlaysv){
        shengmu15VideoPlayer.slowModePlayer(videoPlaysv, getActivity());
        shengmu15VideoPlayer.setSurfaceHolder(videoPlaysv.getHolder(), demoVideoName);
    }
    ...
    private void flashScore(){
        new Thread(new Runnable() {
            ...
            public void run() {
                while (true){
                    double score = shengmu15VideoRecorder.mNetworkFunctions.score;
                    if (score != 0 && shengmu15VideoRecorder.mNetworkFunctions.uploadFlag){
                        finalScore = score;
                        shengmu15VideoRecorder.mNetworkFunctions.score = 0;
