...
public class Duoyinzici6PracticeFragment extends Fragment {
    //示范视频标签
    private String               demoVideoName         = "qilai";
    //上传视频标签
    private String               uploadVideoName       = "duoyinzici6";
    //弹窗对象
    private CongratulationDialog mCongratulationDialog;
    //当前模块对应的视频播放控制器
    private VideoPlayer          duoyinzici6VideoPlayer   = new VideoPlayer();
    //当前模块对应的视频录制控制器
    private VideoRecorder        duoyinzici6VideoRecorder = new VideoRecorder();
    //最后显示在页面上的分数
    private double               finalScore;
    //分数字符串
    private TextView             scoreString;

    public Duoyinzici6PracticeFragment() {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        return inflater.inflate(R.layout.fragment_duoyinzici6_practice, container, false);
    }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        //设置这个模块返回的视频尾部标签
        duoyinzici6VideoRecorder.label = uploadVideoName;
        //相机预览界面设置
        FrameLayout camPreview = view.findViewById(R.id.camera_preview);
        duoyinzici6VideoRecorder.prepareCameraAndStartPreview(getContext(), camPreview);
        //进度条读条&录像倒计时
        ProgressBar progressBar;
        progressBar = view.findViewById(R.id.progress_bar);
        duoyinzici6VideoRecorder.setCountDown(progressBar);
        //分数显示绑定
    ...
    public void onDestroy() {
        super.onDestroy();
        duoyinzici6VideoRecorder.releaseCameraAndStopPreview();
    }
    ...
    private void setReplayVideoBtn(final View replayVideoBtn){
        replayVideoBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                duoyinzici6VideoPlayer.restartVideo(demoVideoName);
            }
        });
    ...
    private void setReplayRecordBtn(final View replayRecordBtn){
        replayRecordBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                duoyinzici6VideoPlayer.stratRecordedPreview();
            }
        });
    ...
    private void setStartPracticeBtn(final View startPracticeBtn){
        startPracticeBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                duoyinzici6VideoRecorder.startMediaCorder(startPracticeBtn);
                duoyinzici6VideoPlayer.recordDone = true;
                waitStatusUpdate();
    ...
    private void setShengmu1VideoPlayer(SurfaceView videoPlaysv){
        duoyinzici6VideoPlayer.slowModePlayer(videoPlaysv, getActivity());
        duoyinzici6VideoPlayer.setSurfaceHolder(videoPlaysv.getHolder(), demoVideoName);
    }
    ...
    private void flashScore(){
        new Thread(new Runnable() {
            ...
            public void run() {
                while (true){
                    double score = duoyinzici6VideoRecorder.mNetworkFunctions.score;
                    if (score != 0 && duoyinzici6VideoRecorder.mNetworkFunctions.uploadFlag){
                        finalScore = score;
                        duoyinzici6VideoRecorder.mNetworkFunctions.score = 0;
