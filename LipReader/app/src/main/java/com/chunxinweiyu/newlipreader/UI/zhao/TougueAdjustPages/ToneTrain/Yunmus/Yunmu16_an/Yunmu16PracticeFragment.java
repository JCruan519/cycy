package com.chunxinweiyu.newlipreader.UI.zhao.TougueAdjustPages.ToneTrain.Yunmus.Yunmu16_an;
...
public class Yunmu16PracticeFragment extends Fragment {
    //示范视频标签
    private String               demoVideoName         = "an";
    //上传视频标签
    private String               uploadVideoName       = "yunmu16";
    //弹窗对象
    private CongratulationDialog mCongratulationDialog;
    //当前模块对应的视频播放控制器
    private VideoPlayer          yunmu16VideoPlayer   = new VideoPlayer();
    //当前模块对应的视频录制控制器
    private VideoRecorder        yunmu16VideoRecorder = new VideoRecorder();
    //最后显示在页面上的分数
    private double               finalScore;
    //分数字符串
    private TextView             scoreString;

    public Yunmu16PracticeFragment() {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        return inflater.inflate(R.layout.fragment_yunmu16_practice, container, false);
    }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        //设置这个模块返回的视频尾部标签
        yunmu16VideoRecorder.label = uploadVideoName;
        //相机预览界面设置
        FrameLayout camPreview = view.findViewById(R.id.camera_preview);
        yunmu16VideoRecorder.prepareCameraAndStartPreview(getContext(), camPreview);
        //进度条读条&录像倒计时
        ProgressBar progressBar;
        progressBar = view.findViewById(R.id.progress_bar);
        yunmu16VideoRecorder.setCountDown(progressBar);
        //分数显示绑定
    ...
    public void onDestroy() {
        super.onDestroy();
        yunmu16VideoRecorder.releaseCameraAndStopPreview();
    }
    ...
    private void setReplayVideoBtn(final View replayVideoBtn){
        replayVideoBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                yunmu16VideoPlayer.restartVideo(demoVideoName);
            }
        });
    ...
    private void setReplayRecordBtn(final View replayRecordBtn){
        replayRecordBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                yunmu16VideoPlayer.stratRecordedPreview();
            }
        });
    ...
    private void setStartPracticeBtn(final View startPracticeBtn){
        startPracticeBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                yunmu16VideoRecorder.startMediaCorder(startPracticeBtn);
                yunmu16VideoPlayer.recordDone = true;
                waitStatusUpdate();
    ...
    private void setShengmu1VideoPlayer(SurfaceView videoPlaysv){
        yunmu16VideoPlayer.slowModePlayer(videoPlaysv, getActivity());
        yunmu16VideoPlayer.setSurfaceHolder(videoPlaysv.getHolder(), demoVideoName);
    }
    ...
    private void flashScore(){
        new Thread(new Runnable() {
            ...
            public void run() {
                while (true){
                    double score = yunmu16VideoRecorder.mNetworkFunctions.score;
                    if (score != 0 && yunmu16VideoRecorder.mNetworkFunctions.uploadFlag){
                        finalScore = score;
                        yunmu16VideoRecorder.mNetworkFunctions.score = 0;
